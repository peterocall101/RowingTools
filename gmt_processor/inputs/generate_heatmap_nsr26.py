...
# Modern template (matches heatmap-marlow26.html). Uses __TOKEN__ placeholders filled
# via str.replace so the embedded JS braces stay literal (no f-string escaping). NSR has
# no per-crew category, so the renderHeatmap cell carries no `.cc` sub-label.
...
def generate_html(rows, comp, title):
    sub = ("GMT% vs WBT. Championship, 2nd and 3rd eights categories. "
           "J14/J15/J16 excluded. Results via regatta.time-team.nl.")
    return (TEMPLATE
