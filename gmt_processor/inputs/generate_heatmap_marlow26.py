#!/usr/bin/env python3
"""
generate_heatmap_marlow26.py
Generate a GMT% heatmap HTML by scraping time-team.nl Marlow Regatta 2026 results.

Marlow Regatta (Sat 20 June 2026, Dorney Lake, 2000m multi-lane) is a one-day,
men's-only, time-trial + multi-lane finals regatta on regatta.time-team.nl, so it
uses the same scraper as NSR/Nottingham/Poplar: each event page has a Time Trial
then Finals A, B, C ...

In 2026 each boat class is a single combined "Championship" event whose finals mix
Championship, Tier and Senior/Junior crews. Every final letter becomes a heatmap
row, every crew is included, and the per-crew band (read from the table's "event"
column, e.g. "T2 - S/J 8's", "J15 2nd 8+") is shown as a label in each cell. 2000m
course, so times are rated directly against the 2000m WBT (no distance scaling).
...
# Ordered by boat: 8+, 4x, 4+, 4-, 2-, 2x, 1x.
# In 2026 each boat class is one combined "Championship" event; its finals mix
# Championship/Tier/Senior-Junior crews and the per-crew band comes from the table's
# "event" column. All crews included. "J14 8x+" is omitted only because there is no
# 8x+ World Best Time to rate it against.
EVENTS = [
    ("ed61ff364-81af-41ec-92ba-da8a734b1ffc", "Ch 8+", "M8+"),
    ("e9b6f8611-9f4f-4157-bb53-8ab4061736ae", "Ch 4x", "M4x"),
    ("e21c7c67c-2b2e-4f66-96ac-28f459ed15bc", "Ch 4+", "M4+"),
    ("e78b0f6cd-ca6d-4bf1-b3cb-dd338d6c5ed8", "Ch 4-", "M4-"),
    ("e07645d98-420a-4462-8550-19a7cae41842", "Ch 2-", "M2-"),
    ("e93582cad-3c06-4490-b507-e953d96075c6", "Ch 2x", "M2x"),
    ("ee3eaf44c-4912-44bf-8a2d-4ab2a556a519", "Ch 1x", "M1x"),
]

# Club code -> name lookup (from regatta.time-team.nl/marlow/2026/results/clubs.php)
CLUBS = {
    "ABS": "Abingdon Sch",
    "AGE": "Agecroft",
    ...
    "IMP": "Imperial Coll",
    "ITC": "Itchen Imperial RC",
    "KCA": "King's Sch Canterbury",
    "KCH": "King's Sch Chester",
    "KCL": "Kings Coll London",
    "KCS": "King's Coll Sch",
    "KGS": "Kingston GS",
    "KHS": "Kew House Sch",
    "KRC": "Kingston",
    "KSE": "King's Sch Ely",
    "KSR": "King's Rochester",
    ...
    "WTN": "Warrington",
    "YRK": "York City",
    "ZAEL": "Ridley Coll (CAN)",
    "ZAMB": "Melbourne Univ (AUS)",
    "ZAQU": "Queens Univ Belfast (IRL)",
    "ZASX": "Sydney (AUS)",
    "ZAUF": "Sydney Univ (AUS)",
    "ZCS": "San Cristoforo, ITA",
    "ZHAE": "Waikato Rowing Club, NZ",
    "ZHEP": "Bates College (USA)",
    "ZHEV": "PNRA, USA",
    "ZHEW": "Fordham Men, USA",
    "ZHEY": "Aramoho Whanganui RC, NZL",
    "ZHZ": "Harvard Univ (USA)",
    "ZXE": "Phillips Exeter Academy (USA)",
}
...
def scrape_event(session, uuid, display_name):
    ...
    for tag in soup.find_all('h2'):
        ...
        # The combined 2026 events carry a per-crew band in an "event" column
        # (e.g. "T2 - S/J 8's", "J15 2nd 8+"). Single-band boats (1x, 2-) omit it.
        # Find its index from the header so we can label each crew in the heatmap.
        cat_idx = None
        header = table.find('tr')
        if header:
            hdr_cells = [c.get_text(strip=True).lower() for c in header.find_all(['th', 'td'])]
            if 'event' in hdr_cells:
                cat_idx = hdr_cells.index('event')
        ...
        for tr in table.find_all('tr'):
            ...
            if cat_idx is not None and cat_idx < len(cells):
                cat = cells[cat_idx].get_text(' ', strip=True)

            # The "crewstroke" column (index 2) holds the club name for crew boats
            # but the athlete's name for singles - capture it so 1x cells can show
            # the sculler (build_races decides whether to use it, by boat class).
  ...
  for(const[ev,races] of groups){
    ...
    for(const r of races){
      ...
      for(let i=0;i<maxP;i++){
        ...
        const cat=l.cat?`<span class="cc">${l.cat}</span>`:'';
        cells+=`<td style="background:${b};${dim}"><div class="cell"><span class="cn">${l.crew}</span><span class="cp" style="color:${f}">${p}</span><span class="ct">${l.time}</span>${cat}</div></td>`;
      }
      tb+=`<tr>${cells}</tr>`;
    }
...
def generate_html(rows, comp, title):
    sub = ("GMT% vs WBT. Men's one-day regatta over 2000m at Dorney Lake. Each boat "
           "class is one combined Championship event; its finals (A, B, C ...) mix "
           "Championship, Tier and Senior/Junior crews, and each crew's band is "
           "labelled in its cell. All crews included. Results via regatta.time-team.nl.")
    return (TEMPLATE
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Generate GMT% heatmap HTML from Marlow Regatta 2026 results.")
    parser.add_argument("--comp",  default="marlow26",            help="Competition code for CSV filenames")
    parser.add_argument("--title", default="Marlow Regatta 2026", help="Page title")
