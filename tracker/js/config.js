...
// `let`, not `const`: sample mode swaps this for the localStorage-backed
// stand-in in trial.js once boot knows there is no signed-in session. Every
// caller reads `sb` at call time, so the swap is invisible to the rest of the
// app. login.html loads this file without trial.js and never reassigns it.
