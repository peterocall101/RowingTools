#!/usr/bin/env python3
"""
generate_heatmap_sheet.py
Generate a GMT% heatmap HTML from a Google Sheets CSV export.

Format expected: Race, Event, Type, Name, Position, Lane, Time, Diff
Only "Final" rows are used. J16 (and below) excluded. Position 9999 excluded.

Usage:
    python generate_heatmap_sheet.py --csv temp_results.csv --comp durham25 --title "Durham Regatta 2025"
    python generate_heatmap_sheet.py --csv temp_results.csv --comp durham25 --title "Durham Regatta 2025" --out ../../heatmap-durham25.html
"""
...
EXCLUDE_RE = re.compile(r'\bJ(?!18)\d{2}\b', re.I)
...
def event_to_boat(ev):
    if EXCLUDE_RE.search(ev):
        return None
    gender = 'W' if re.match(r'^W[.\s]', ev.strip()) else 'M'
    is_lwt = bool(re.search(r'\bLwt\b', ev, re.I))
    prefix = ('LW' if gender == 'W' else 'LM') if is_lwt else gender
    if re.search(r'8\+', ev):   return prefix + '8+'
    if re.search(r'4x',  ev):   return prefix + '4x'
    if re.search(r'4\+', ev):   return prefix + '4+'
    if re.search(r'4-',  ev):   return prefix + '4-'
    if re.search(r'2-',  ev):   return prefix + '2-'
    if re.search(r'2x',  ev):   return prefix + '2x'
    if re.search(r'1x',  ev):   return prefix + '1x'
    return None


def event_display_name(ev):
    parts = re.split(r'[.\s]+', ev.strip())
    GENDER = {'Op': 'Open', 'W': 'W', 'Mxd': 'Mixed'}
    CAT    = {'Ch': 'Champ', 'Int': 'Int', 'Dev': 'Dev',
              'J18': 'J18', 'J16': 'J16', 'Lwt': 'Lwt'}
    BOAT   = {'1x': '1x', '2x': '2x', '4x-': '4x', '4x': '4x',
              '4-': '4-', '4+': '4+', '2-': '2-', '8+': '8+'}
    out = []
    for p in parts:
        if p in GENDER: out.append(GENDER[p])
        elif p in CAT:  out.append(CAT[p])
        elif p in BOAT: out.append(BOAT[p])
        else:           out.append(p)
    return ' '.join(out)
...
def parse_crew(name):
    name = name.strip()
    # Strip trailing composite marker "C" after closing paren
    clean = re.sub(r'\)\s+C$', ')', name)
    m = re.match(r'^(.+?)\s*\([^)]+\)$', clean)
    club = m.group(1).strip() if m else clean
    return club, name


def build_races(csv_path, wbt):
    ...
    with open(csv_path, newline='', encoding='utf-8') as f:
        ...
        for row in reader:
            ...
            if pos <= 0 or pos == 9999:
                continue
            ev = row['Event'].strip()
            if event_to_boat(ev) is None:
                continue
            key = (int(row['Race']), ev)
            groups[key].append(row)

    rows = []
    for (race_num, ev), entries in sorted(groups.items()):
        boat  = event_to_boat(ev)
        wbt_t = wbt.get(boat)
        lanes = []
        for r in sorted(entries, key=lambda x: int(x['Position'])):
            t   = parse_time(r['Time'])
            pct = round(wbt_t / t * 100, 1) if (wbt_t and t) else None
            club, crew = parse_crew(r['Name'])
            lanes.append({
                "crew": crew,
                "club": club,
                "time": fmt_time(t) if t else r['Time'].strip(),
                "pct":  pct,
            })
        if lanes:
            rows.append({
                "event": event_display_name(ev),
                "round": "Final",
                "lanes": lanes,
                "boat":  boat or "",
            })

    # Sort: Women's first, then Open/Men's, then event name
...
<body>
<h1>{title}</h1>
<p class="sub">GMT% vs WBT. Dev, Int, Champ and Open categories. J18 events included. Heats excluded.</p>
<div class="tabs">
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Generate GMT% heatmap HTML from a CSV sheet export.")
    parser.add_argument("--csv",   required=True,             help="Path to input CSV file")
    parser.add_argument("--comp",  default="regatta25",       help="Competition code for CSV filenames")
    parser.add_argument("--title", default="Regatta 2025",    help="Page title")
